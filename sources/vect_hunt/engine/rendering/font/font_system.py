...
class FontSystem:
    ...
    def __init__(self, config: dict[str, Any]) -> None:
        """
        Initialise le FontSystem avec la configuration des polices.

        Parameters
        ----------
        config : dict[str, Any]
            Configuration chargee depuis fonts.json.
        """
        self._config = config
    ...
    def _load_config(self) -> None:
        ...
        """
        for style_name, style_def in self._config.items():
            ...
            self._styles[style_name] = FontStyle(font, color, antialias)
    ...
    def get(self, style_name: str) -> FontStyle:
        ...
        if style_name not in self._styles:
            raise KeyError(
                ...
                f"Styles disponibles : {list(self._styles.keys())}"
            )
